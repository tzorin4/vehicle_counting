...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Copy a random subset of image-label pairs with train/val split")
    ...
    parser.add_argument("--seed", "-s", type=int, default=None, help="Random seed for reproducibility (optional)")
    ...
    all_pairs = load_img_and_lbl(img_dir, lbl_dir)
    print(f"here {args.seed}")
    total_available = len(all_pairs)

    # Randomly shuffle the list of pairs
    random.shuffle(all_pairs)

    # Take the first num_pairs from the shuffled list
    number_of_pairs_to_copy = min(args.num_pairs, total_available)
    selected_pairs = all_pairs[:number_of_pairs_to_copy]

    for img_path, lbl_path in tqdm(selected_pairs):
    ...
    print(f"Copied {number_of_pairs_to_copy} random pairs from {total_available} total to {output_dir} "
          f"with train/val split of {train_split*100:.0f}/{(1-train_split)*100:.0f}.")